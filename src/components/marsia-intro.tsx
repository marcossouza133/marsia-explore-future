...
import { AudioLines, ChevronRight, Hexagon } from "lucide-react";
...
export function MarsiaIntro({ active, onComplete }: { active: boolean; onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    signals.forEach((_, index) => {
      timersRef.current.push(window.setTimeout(() => setRevealed(index + 1), 1700 + index * 1500));
    });
    timersRef.current.push(window.setTimeout(() => finish(), 7200));
  ...
  return (
    <div className={`marsia-intro ${leaving ? "marsia-intro-leaving" : ""}`} role="dialog" aria-label="Inicialização da MARSIA">
      ...
      <div className="marsia-intro-core">
        ...
        {audioBlocked && (
          <button type="button" className="marsia-intro-start" onClick={startAudio}>
            <AudioLines size={16} /> TOQUE PARA INICIAR
          </button>
        )}
      </div>
