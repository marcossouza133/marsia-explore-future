...
const MARKET_KEY = "marsia-market-v1";

type SavedListing = Omit<Listing, "icon">;

function loadSavedListings(): Listing[] {
  try {
    const raw = localStorage.getItem(MARKET_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as SavedListing[];
    return parsed.map((l) => ({ ...l, icon: icons[l.category] ?? Gem }));
  } catch { return []; }
}

export function MarketplacePage() {
  const { territories, resources, playerName } = useMarsia();
  const [listings, setListings] = useState<Listing[]>(() => [...loadSavedListings(), ...initialListings]);
  const [filter, setFilter] = useState<(typeof filters)[number]>("Todos");
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ title: "", category: "Território" as Category, region: "Elysium", price: "", desc: "", link: "" });

  useEffect(() => {
    const mine = listings.filter((l) => l.mine).map(({ icon: _icon, ...rest }) => rest);
    try { localStorage.setItem(MARKET_KEY, JSON.stringify(mine)); } catch { /* armazenamento indisponível */ }
  }, [listings]);

  const visible = filter === "Todos" ? listings : listings.filter((l) => l.category === filter);

  const linkOptions = [
    ...territories.map((t) => ({ value: `territory:${t.id}`, label: `Meu território — ${t.name} (${t.code})` })),
    ...resources.map((r) => ({ value: `resource:${r.id}`, label: `Recurso — ${r.name} x${r.amount}` })),
  ];

  const applyLink = (value: string) => {
    setForm((current) => {
      const next = { ...current, link: value };
      if (value.startsWith("territory:")) {
        const t = territories.find((item) => `territory:${item.id}` === value);
        if (t) {
          next.title = t.name;
          next.category = "Território";
          next.region = t.region;
        }
      }
      if (value.startsWith("resource:")) {
        const r = resources.find((item) => `resource:${item.id}` === value);
        if (r) {
          next.title = r.name;
          next.category = "Recurso";
        }
      }
      return next;
    });
  ...
  const submit = () => {
    ...
    const linkedTerritory = form.link.startsWith("territory:") ? territories.find((t) => `territory:${t.id}` === form.link) : undefined;
    const linkedResource = form.link.startsWith("resource:") ? resources.find((r) => `resource:${r.id}` === form.link) : undefined;
    setSaving(true);
    setTimeout(() => {
      const seq = listings.length + 1;
      setListings((list) => [{
        id: `mine-${Date.now()}`,
        code: linkedTerritory ? linkedTerritory.code : `AN-${String(seq).padStart(3, "0")}`,
        ...
        mine: true,
        status: "ativo",
        meta: linkedTerritory
          ? `Nível ${linkedTerritory.level} · ${linkedTerritory.structures.length} estrutura(s) · Defesa ${linkedTerritory.defense}`
          : linkedResource
            ? `Quantidade disponível: ${linkedResource.amount}`
            : undefined,
      }, ...list]);
      setSaving(false);
      setOpen(false);
      setFilter("Todos");
      setForm({ title: "", category: "Território", region: "Elysium", price: "", desc: "", link: "" });
  ...
  return <>
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {visible.map(({ id, code, title, category, region, price, seller, rarity, desc, icon: Icon, mine, meta, status }) => (
        <Panel key={id} className={`flex min-h-[320px] flex-col p-6 transition-colors hover:border-primary/40 ${mine ? "border-primary/40" : ""}`}>
          ...
          {meta && <p className="mt-3 border-l-2 border-primary bg-primary/5 p-2 text-[10px] uppercase tracking-[.1em] text-primary">{meta}</p>}
          <div className="mt-auto pt-6">
            ...
            {mine && <div className="mt-3 text-center font-mono text-[9px] uppercase tracking-[.14em] text-signal">Status: {status ?? "ativo"}</div>}
          </div>
        </Panel>
      ))}
    </div>
        ...
        <div className="w-full max-w-lg border border-border bg-card p-7" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mt-6 space-y-4">
            {linkOptions.length > 0 && (
              <div>
                <label className="text-[10px] uppercase tracking-[.15em] text-muted-foreground">Vincular item seu (opcional)</label>
                <select value={form.link} onChange={(e) => applyLink(e.target.value)} className="mars-input">
                  <option value="">Anúncio livre</option>
                  {linkOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <p className="mt-2 text-[10px] leading-4 text-muted-foreground">Somente territórios e recursos que pertencem a você podem ser vinculados.</p>
              </div>
            )}
            <div>
