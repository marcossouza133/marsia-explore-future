...
export function CommsRoom({ name, count }: { name: string; count: number }) {
  return <button className="group flex w-full items-center gap-4 border-b border-border py-4 text-left transition-colors hover:text-primary"><span className="grid size-9 place-items-center border border-border bg-muted"><Radio size={15}/></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{name}</span><span className="mt-1 block text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Canal aberto</span></span><span className="font-mono text-xs text-signal">{count} online</span></button>
}
